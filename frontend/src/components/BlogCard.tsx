...
export const BlogCard = ({
  ...
  return (
    <Link to={`/blogs/${id}`}>
      <div className="p-4 w-screen border-b border-slate-200 max-w-screen-md cursor-pointer">
        <div className="flex">
          ...
          <div className="text-sm font-thin text-slate-500 pl-2 flex justify-center flex-col">
            {publishedDate}
          </div>
        </div>
        <div className="font-semibold text-xl pt-2">{title}</div>
        <div className="font-thin text-md">{content.slice(0, 100) + "..."}</div>
        <div className="text-sm text-slate-500 font-thin pt-4">{`${Math.ceil(content.length / 100)} minute(s) read`}</div>
      </div>
    </Link>
  );
};
